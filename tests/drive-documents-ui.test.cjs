...
function harness({save,list}={}){
 ...
 ctx.DriveDocuments.save=async()=>{writes++;const saved=save?await save():row;serverRows.push(saved);return saved;};ctx.DriveDocuments.list=list||async function(){return serverRows;};
...
test('pending duplicate clicks create one link and a confirmed result appears in both views',async()=>{
 let done;const h=harness({save:()=>new Promise(r=>done=r)});const pending=h.ctx.saveDriveDoc();await h.ctx.saveDriveDoc();assert.equal(h.writes(),1);assert.equal(h.elements['drive-save'].disabled,true);done(row);await pending;assert.equal(h.rows().length,1);assert.equal(h.elements['drive-save'].disabled,false);assert.ok(h.closed.includes('m-drive-doc'));assert.match(h.elements['inv-drive-list'].innerHTML,/test_file_1234567890/);assert.match(h.elements['docs-drive-list'].innerHTML,/TEST_001/);assert.doesNotMatch(h.elements['inv-drive-list'].innerHTML,/<img/);
});
...
test('a pre-save read finishing late cannot erase the confirmed link',async()=>{
 let done,reads=0;const h=harness({list:()=>++reads===1?new Promise(r=>done=r):Promise.resolve([row])});const pending=h.ctx.loadDriveDocs();await h.ctx.saveDriveDoc();done([]);await pending;assert.equal(h.rows().length,1);
});
...
test('saving before the initial read completes refreshes the full list instead of declaring a partial cache complete',async()=>{
 ...
 assert.equal(h.rows().length,2);assert.match(h.elements['docs-drive-list'].innerHTML,/Existing.pdf/);
});
