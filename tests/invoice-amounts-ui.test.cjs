...
function fixture({execute=async()=>({id:'request',status:'pending'}),timeoutMs}={}){
 const nodes={},calls=[],notices=[];let auth;
 ...
 const client={row:async(table,action,key,values,before)=>({table,action,key,values,before}),submit:async ops=>{calls.push(ops);return execute();}};
 ...
 return {ui,node,nodes,calls,notices,line,auth:(...args)=>auth(...args)};
}
