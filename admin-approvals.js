(function(root,factory){
  ...
  Object.assign(labels,{contacts:'연락처',company:'업체명',person:'담당자 이름',person_en:'영문 이름',duty:'담당 업무',department:'부서',mobile:'휴대전화',fax:'팩스',messenger:'메신저',website:'웹사이트',tags:'태그'});
  Object.assign(labels,{contracts:'계약서',contract_type:'계약유형',counterparty_contact:'거래처 담당자',start_date:'계약 시작일',end_date:'계약 종료일',notice_date:'갱신거절 통보기한',auto_renewal:'자동갱신',renewal_terms:'갱신 조건',previous_contract_id:'이전 계약',territory:'대상 국가',channels:'판매채널',exclusive:'독점 여부',price_terms:'공급가격 기준',minimum_order:'최소발주',incoterms:'인도조건',annual_commitment:'연간 구매약정',exclusivity_terms:'독점 유지 조건',payment_type:'수금 방식',deposit_pct:'선금 비율 (%)',balance_pct:'잔금 비율 (%)',deposit_due:'선금 지급 기준·기한',balance_due:'잔금·후불 지급 기준·기한',bank_fee:'송금수수료 부담',payment_terms:'수금 특약',foc_status:'FOC 유무',foc_terms:'FOC 제공 조건',foc_products:'FOC 대상 제품',foc_limit:'FOC 한도',document_id:'계약서 서명본',amendment_document_id:'부속합의서',special_terms:'주요 특약'});
  ...
  function renderDiff(operation){
    ...
    const rows=keys.map(key=>{
      ...
      return `<tr${changed?' class="changed"':''}><th scope="row">${escape(op.table==='contacts'&&key==='title'?'직책':field(key))}${changed?' <span class="changed-label">변경</span>':''}</th><td><pre>${escape(valueText(old))}</pre></td><td><pre>${escape(valueText(next))}</pre></td></tr>`;
    }).join('');
  ...
  function requestTitle(r,state){
    const ops=Array.isArray(r.operations)?r.operations:[],op=ops[0]||{};
    const value={...op.before,...op.values,...op.invoice};
    if(op.table==='contacts')return {customer:value.company||'',title:`${value.company||'업체 미확인'} · ${value.person||'담당자 미확인'}${ops.length>1?` 외 ${ops.length-1}건`:''}`,kind:'연락처',count:ops.length};
    const customer=state.customerNames?.[value.customer_id]||value.customer||'';
    const title=value.name||value.no||value.invoice_no||value.title||r.reason||'변경 요청';
    return {customer,title:typeof title==='string'?title:valueText(title),kind:op.action==='invoice'?'주문':field(op.table||''),count:ops.length};
  }
  Object.assign(labels,{"kol_support_status":"KOL 비용지원 조항","kol_support_terms":"KOL 비용지원 조건·근거","vmd_support_status":"집기·VMD 비용지원 조항","vmd_support_terms":"집기·VMD 비용지원 조건·근거","logistics_status":"물류·선적 조항","logistics_terms":"물류·선적 조건·근거","certification_status":"수출 인증 조항","certification_terms":"수출 인증 조건·근거","document_handover_status":"공식 서류·인증 이관 조항","document_handover_terms":"공식 서류·인증 이관 조건·근거","sns_handover_status":"공식 SNS·계정 이관 조항","sns_handover_terms":"공식 SNS·계정 이관 조건·근거"});
  Object.assign(labels,{"returns_status":"교환·반품 조항","returns_terms":"교환·반품 조건·근거","defect_liability_status":"하자 책임 조항","defect_liability_terms":"하자 책임 조건·근거","unclear_cause_status":"원인 불명 처리 조항","unclear_cause_terms":"원인 불명 처리 조건·근거"});
  function contractOverview(operations){
    return (Array.isArray(operations)?operations:[]).filter(op=>op.table==='contracts').map(op=>{
      const v={...op.before,...op.values};
      const groups=[['계약기간',[['기간',`${v.start_date||'시작일 미확정'} ~ ${v.end_date||'종료일 미확정'}`],['상태',v.status],['자동갱신',v.auto_renewal],['통보기한',v.notice_date],['갱신 조건',v.renewal_terms]]],['계약조건',['territory','channels','exclusive','price_terms','minimum_order','incoterms','annual_commitment','exclusivity_terms'].map(k=>[field(k),v[k]])],['수금조건',['payment_type','currency','payment_method','deposit_pct','balance_pct','deposit_due','balance_due','bank_fee','payment_terms'].map(k=>[field(k),v[k]])],['FOC',['foc_status','foc_terms','foc_products','foc_limit'].map(k=>[field(k),v[k]])],['KOL 비용지원',['kol_support_status','kol_support_terms'].map(k=>[field(k),v[k]])],['집기·VMD 비용지원',['vmd_support_status','vmd_support_terms'].map(k=>[field(k),v[k]])],['물류·선적',['logistics_status','logistics_terms'].map(k=>[field(k),v[k]])],['수출 인증',['certification_status','certification_terms'].map(k=>[field(k),v[k]])],['공식 서류·인증 이관',['document_handover_status','document_handover_terms'].map(k=>[field(k),v[k]])],['공식 SNS·계정 이관',['sns_handover_status','sns_handover_terms'].map(k=>[field(k),v[k]])],['교환·반품',['returns_status','returns_terms'].map(k=>[field(k),v[k]])],['하자 책임',['defect_liability_status','defect_liability_terms'].map(k=>[field(k),v[k]])],['원인 불명 처리',['unclear_cause_status','unclear_cause_terms'].map(k=>[field(k),v[k]])],['주요 특약 · 원문',[['특약',v.special_terms]]]];
      return `<section class="contract-overview"><h3>${escape(v.name||'계약 정보')}${op.action==='delete'?' · 삭제 대상':''}</h3><div class="contract-groups">${groups.map(([title,items])=>`<section><h4>${escape(title)}</h4><dl>${items.filter(([,value])=>value!==null&&value!==undefined&&value!=='').map(([label,value])=>`<div><dt>${escape(label)}</dt><dd>${escape(value)}</dd></div>`).join('')||'<div><dd>미확정</dd></div>'}</dl></section>`).join('')}</div></section>`;
    }).join('');
  }
  function contactsOverview(operations){
    const contacts=(Array.isArray(operations)?operations:[]).filter(op=>op.table==='contacts');
    if(!contacts.length)return '';
    return `<section class="contacts-overview"><h3>연락처 ${contacts.length}건</h3><div class="diff-scroll" tabindex="0" role="region" aria-label="연락처 일괄 변경 요약"><table class="diff"><thead><tr><th>변경</th><th>업체 · 담당자</th><th>연락수단</th><th>상태</th></tr></thead><tbody>${contacts.map(op=>{const v={...op.before,...op.values};return `<tr><td>${escape({insert:'추가',update:'수정',delete:'삭제'}[op.action]||'변경')}</td><td>${escape(v.company||'')} · ${escape(v.person||'')}</td><td><pre>${escape(['mobile','phone','email','messenger'].filter(k=>v[k]).map(k=>`${field(k)}: ${v[k]}`).join('\n')||'—')}</pre></td><td>${escape(v.status||'활성')}</td></tr>`;}).join('')}</tbody></table></div></section>`;
  }
  ...
  function createController({rpc,onChange=()=>{},pageSize=50}){
    const state={requests:[],isAdmin:false,notes:Object.create(null),expanded:Object.create(null),customerNames:Object.create(null),filter:'all',offset:0,pageSize,loading:false,reviewing:false,error:'',message:''};
    ...
    function reset(){generation++;sessionEpoch++;Object.assign(state,{requests:[],isAdmin:false,loading:false,reviewing:false,notes:Object.create(null),expanded:Object.create(null),customerNames:Object.create(null),error:'',message:''});notify();}
  ...
  function mount({document,client,confirm=message=>confirmReview(document,message)}){
    ...
    host.addEventListener('toggle',event=>{const el=event.target;if(el.matches('[data-request]')&&host.contains(el))controller.state.expanded[el.dataset.request]=el.open;},true);
    ...
    async function start(){
      ...
      try{
        ...
        // Resolve names only after authentication; a failed lookup must not hide requests.
        if(client.from){try{const result=await client.from('customers').select('id,name');if(ticket===sessionGeneration&&!result.error&&Array.isArray(result.data)){controller.state.customerNames=Object.fromEntries(result.data.map(c=>[c.id,c.name]));host.innerHTML=render(controller.state);}}catch{ /* Request titles remain available without names. */ }}
