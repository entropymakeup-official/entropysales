...
function dashboardControls(summary){
  ...
  return `<section class="card dash-filters" aria-label="대시보드 조회 기간">
    <form onsubmit="event.preventDefault();applyDashboardPeriod()">
      <label>연도<select id="dash-year" onchange="setDashboardYear(this.value)">${years.map(y=>`<option ${y===period.start.slice(0,4)?'selected':''}>${y}</option>`).join('')}</select></label>
      ...
      <label>거래처<select id="dash-customer" onchange="applyDashboardPeriod()"><option value="">전체 거래처</option>${customers.map(c=>`<option value="${esc(c)}" ${c===period.customer?'selected':''}>${esc(c)}</option>`).join('')}</select></label>
    ...
    <p class="dash-period-summary">${period.start} ~ ${period.end} · ${esc(period.customer||'전체 거래처')} · 발주일 기준 · 주문 ${summary.rows.length}건</p>
    <details><summary>매출 집계 기준 보기</summary>
      <p>취소(Cancelled) 주문은 제외합니다. 매출은 Paid 품목의 공급가액 기준이며 입금액이 아닙니다. 거래처 마스터의 영세·과세를 기본값으로 사용합니다.</p>
...
function setDashboardYear(year){_dashPeriod={...dashboardPeriod(),start:year+'-01-01',end:year+'-12-31'};renderDash();}
function setDashboardMonth(){
  const date=today(),year=Number(date.slice(0,4)),month=Number(date.slice(5,7));
  _dashPeriod={...dashboardPeriod(),start:date.slice(0,7)+'-01',end:date.slice(0,7)+'-'+new Date(year,month,0).getDate()};renderDash();
}
function dashboardMetricCard(metric,label,value,sub=''){
  return `<button type="button" class="kpi dash-metric ${metric==='revenue'?'dash-primary':''}" onclick="openDashboardEvidence('${metric}')"><span class="lbl">${label}</span><span class="val">${fmt(value)}</span><span class="sub">${sub||'근거 주문 보기 →'}</span></button>`;
}
function dashboardGoalCard(){
  const goal=5000000000,p=dashboardPeriod();
  const eligible=p.start==='2026-01-01'&&p.end==='2026-12-31'&&!p.customer;
...
function openDashboardEvidence(metric){
  ...
  const labels={revenue:'공급가액 집계분',pendingRevenue:'세금 기준 확인 필요 금액',foc:'무료 제공 평가금액',lost:'Lost 금액'};
  ...
  dialog.innerHTML=`<div class="mhd"><h2>${labels[metric]} 근거 주문</h2><button class="btn" onclick="closeDashboardEvidence()" autofocus>닫기</button></div>
    ...
    <p>${metric==='pendingRevenue'?'아래 금액은 저장된 Paid 기록액이며 공급가액으로 확정되지 않았습니다. 세금 구분·단가 기준을 확인할 증빙이 필요합니다.':'각 금액은 이 카드에 포함된 품목만 집계합니다.'} 주문 상세에서는 전체 품목을 확인할 수 있습니다.</p>
